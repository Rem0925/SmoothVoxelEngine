...
void main()
{
    ...
    // Calcular la onda base
    float wave = (sin(time * 1.5 + baseWorldPos.x * 2.0) * cos(time * 1.2 + baseWorldPos.z * 2.0) + 1.0) * 0.5;
    
    // Afilar la cresta de la ola
    wave = pow(wave, 1.5);
    ...
    animatedPos.y += wave * 0.2 * shoreMask;
