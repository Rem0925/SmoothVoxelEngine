...
bool draw_button_at(const char* text, int x, int y, int w, int h, bool active = false) {
    ...
    Color bg = active ? MenuTheme::col_btn_active : (hovered ? MenuTheme::col_btn_hover : MenuTheme::col_btn);
    Color border = active ? MenuTheme::col_btn_active_line : MenuTheme::col_btn_line;
...
void MenuResourcePacks::draw(MenuManager& manager) {
    ...
    DrawText(title, px + (panel_w - title_w) / 2, py + 18, 26, MenuTheme::col_title);

    if (packs.empty()) {
        const char* empty_msg = "No se encontraron paquetes en 'resourcepacks/'";
        int msg_w = MeasureText(empty_msg, 16);
        DrawText(empty_msg, px + (panel_w - msg_w) / 2, py + 80, 16, MenuTheme::col_desc);

        const char* hint = "Coloca una carpeta de paquete en la carpeta resourcepacks/";
        int hint_w = MeasureText(hint, 14);
        DrawText(hint, px + (panel_w - hint_w) / 2, py + 110, 14, Fade(MenuTheme::col_desc, 0.6f));
    } else {
        int list_x = px + 12;
        int list_y = py + 58;
        ...
        for (int i = 0; i < (int)packs.size(); ++i) {
            ...
            Color bg = hovered ? MenuTheme::col_pack_hover : MenuTheme::col_pack_bg;
            ...
            DrawRectangleRoundedLines({ (float)list_x, (float)iy, (float)list_w, (float)item_h }, 0.15f, 4,
                p.selected ? MenuTheme::col_btn_active_line : Color{ 60, 70, 85, 255 });
            ...
            if (p.icon.id != 0) {
                ...
                DrawText("?", ix + icon_size / 2 - 5, iy_icon + icon_size / 2 - 9, 18, MenuTheme::col_desc);
            }
            ...
            if (!p.description.empty()) {
                DrawText(p.description.c_str(), text_x, iy + 34, 12, MenuTheme::col_desc);
            }
